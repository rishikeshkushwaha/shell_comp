from pyomo.environ import ConcreteModel, Var, Objective, NonNegativeReals, Constraint, PositiveIntegers
...
M.scs = Var(M.J, within=PositiveIntegers)
M.fcs = Var(M.J, within=PositiveIntegers)
...
def obj_expression(M):
    return a*quicksum(M.ds[i, j]*M.dist[i, j] for i in M.I for j in M.J) + c*quicksum(M.scs[j] + r*M.fcs[j] for j in M.J)
...
def c6(M,i):
    return quicksum(M.ds[i,j] for j in M.J) == forecast_demand_dict['2020'][i]
...
result_data = []
for j in M.J:
    result_data.append({"data_type":'SCS',"demand_point_index":None,"supply_point_index": j, "value": M.scs[j].value,})
for j in M.J:
    result_data.append({"data_type":'FCS',"demand_point_index":None,"supply_point_index": j, "value": M.fcs[j].value,})
for i in M.I:
    for j in M.J:
            result_data.append({"data_type":'DS',"demand_point_index": i, "supply_point_index": j, "value": M.ds[i,j].value,})
